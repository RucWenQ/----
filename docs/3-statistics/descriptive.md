...
1. **它是后续一切分析的前置**。配对 t 检验要看差值的分布，ANOVA 和回归要检查模型残差；回归还要查异常点与共线性。元分析则常要读取论文里的均值和标准差。这些都依赖描述性统计先把数据"摸清楚"。
2. **它是论文里的标配**。任何一篇规范的实证心理学论文，Method 节里都有样本特征段，Results 节开头都有变量描述/相关矩阵表。<span class="kw">没写好描述性统计，审稿人会直接判断你"不熟悉学科规范"。</span>
3. **它最容易暴露问题**。均值超出量表范围、SD 接近 0、分布出现长尾或孤立点，都会提示先核对编码、测量和模型，而不是直接进入显著性检验。
...
- 显著性星号一般两两相关全做双侧检验。
- 偏度和峰度**不是必报**，也不能单独决定能否使用参数方法；它们应与分布图、异常点、样本量、研究设计和模型残差一起判断。
  :::
...
::: tip 经验法则
反应时（RT）、收入、社交关系数等常见右偏变量，优先同时查看**中位数与 IQR**；是否还报告均值、使用变换或改用稳健模型，要由研究问题和后续模型决定。李克特单题通常先报各类别频数或中位数；多题合成分能否用均值，要结合量表构造和实际分布判断。类别变量用频数、比例和众数描述。
:::
...
- **峰度**（kurtosis）主要反映尾部厚度和极端值倾向。要先确认软件口径：pandas 的 `Series.kurt()` 报 Fisher **超额峰度**（正态分布为 0），有些软件或公式则报 Pearson 峰度（正态分布为 3）。因此表格必须写清是哪一种定义。

::: warning 不把经验阈值当通行证
`|偏度| < 2`、`|超额峰度| < 7` 常被当作经验筛查线，但它们不是所有参数方法共用的正态性判据，也不能推出“满足阈值即可使用参数检验”。同样，超出阈值也不等于必须转换数据或改做秩检验。

按下面的顺序判断：

- 先画直方图、Q-Q 图和箱线图，核对长尾、离群点、取值边界与录入错误。
- 再检查目标方法真正要求近似正态的量：配对 t 检验看**差值**，ANOVA 看组内误差，回归看给定预测变量后的误差，而不是要求每个原始变量都正态。
- 结合样本量、组间是否平衡、方差是否接近以及异常点影响；必要时比较原模型与稳健方法、变换或重抽样结果。
...
```python [pingouin]
# 对每列运行 Shapiro-Wilk 检验，返回 W、p 值和按 alpha 判断的 normal 列
...
先明确要检查什么。单样本 t 检验关注观测值相对总体均值的误差，配对 t 检验关注配对差值，独立样本 t 检验与 ANOVA 关注各组误差，线性回归关注给定预测变量后的误差。描述每个原始变量的形状仍然有用，但“某个变量不正态”不等于模型推断必然无效。

**图形、数值和模型诊断结合使用**：

- 直方图和箱线图：识别偏态、边界、混合分布与孤立点。
- Q-Q 图（quantile-quantile plot）：重点看系统性弯曲和尾部偏离，不要求所有点精确贴线。
- 偏度、超额峰度及其软件定义：用于描述偏离的方向和程度，不作单一判定线。
- Shapiro-Wilk：检验“数据来自正态总体”这一精确零假设；_p_ 值反映样本量与偏离程度的共同作用，不是非正态程度的效应量。

::: tip 实操建议
不存在“Shapiro-Wilk 只适合 _n_ < 50”或“_N_ > 200 几乎一定显著”的通用分界。截至 2026-08-11，SciPy 1.18.0 的 `scipy.stats.shapiro` 要求至少 3 个观测，并提示 _N_ > 5000 时 _W_ 统计量仍准确，但 _p_ 值可能不准确。

大样本下检验能发现很小的偏离，小样本下则可能漏掉有影响的偏离。<span class="kw">不要用一次正态性检验替代残差图、异常点诊断和敏感性分析。</span> t 检验、ANOVA 与回归在一些大样本、近似平衡且无强影响点的情形下对非正态具有一定稳健性；方差不齐、组样本量失衡、重尾或强异常点同时出现时，不能沿用这个结论。
:::
...
- Kline, R. B. (2016). _Principles and practice of structural equation modeling_ (4th ed.). Guilford Press. ——SEM 中的分布诊断与估计方法参考。
...
- Shapiro, S. S., & Wilk, M. B. (1965). An analysis of variance test for normality (complete samples). _Biometrika, 52_(3/4), 591–611. https://doi.org/10.1093/biomet/52.3-4.591
- Lumley, T., Diehr, P., Emerson, S., & Chen, L. (2002). The importance of the normality assumption in large public health data sets. _Annual Review of Public Health, 23_, 151–169. https://doi.org/10.1146/annurev.publhealth.23.100901.140546
- Blanca, M. J., Alarcón, R., Arnau, J., Bono, R., & Bendayan, R. (2017). Non-normal data: Is ANOVA still a valid option? _Psicothema, 29_(4), 552–557. https://doi.org/10.7334/psicothema2016.383
- SciPy 1.18.0 documentation. `scipy.stats.shapiro`（核查于 2026-08-11）. https://docs.scipy.org/doc/scipy/reference/generated/scipy.stats.shapiro.html
